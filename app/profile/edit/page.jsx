...
const AvailableDaysForm = () => {
  ...
  // Fetch stored days using Capacitor Preferences
  const fetchStoredDays = async () => {
    try {
      const { value } = await Preferences.get({ key: "selectedDays" });
      if (value) {
        setSelectedDays(JSON.parse(value));
      }
    } catch (error) {
  ...
  const handleToggleDay = (day) => {
    setSelectedDays((prev) => {
      ...
      // Save updated days immediately after update
  ...
  useEffect(() => {
    fetchStoredDays(); // Fetch stored days when the component is mounted
  }, []);
...
export default function ProfileEdit({ userId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage(
          "Profile updated successfully! Changes will reflect in a while"
        );
      } else {
