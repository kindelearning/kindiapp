...
export default function NewCalendar({ activities }) {
  ...
  const handleTouchMove = useCallback((event) => {
    ...
    if (Math.abs(dx) > 5 || Math.abs(dy) > 5) {
      event.preventDefault(); // Only prevent when there's significant movement
    }
  }, []);
  ...
  const handleDraggableTouchEnd = useCallback(
    async (event) => {
      ...
      // Look for the closest CalendarDay element with data-date
      ...
      if (dropZone) {
        ...
        if (dateString) {
          ...
          console.error("Drop zone has no valid date attribute.");
        }
      } else {
        console.error("No valid drop zone detected.");
      }
